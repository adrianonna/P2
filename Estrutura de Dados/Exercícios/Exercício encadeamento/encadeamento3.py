...
class Lista:
    ...
    def Tamanho(self):
        valorCorrente = self._inicio
        cont = 1
        if valorCorrente == None:
            return 'Tamanho: Lista vazia!'
        else:
            while valorCorrente.get_proximo() != None:
              cont += 1
            return 'Tamanho: ' + cont
    ...
    def Inserir(self, dado, posicao): #inserir depois do valor informado, se houver dois iguais, depois do primeiro que seja igual
        ...
        while valorCorrente != None:
            if cont == posicao: #se o dado do objeto for igual ao valor informado
...
while opcao != 8:
    ...
    if opcao == 1:
        ...
        valorCorrente = input('Informe a posição onde o elemento irá ser inserido: ')
